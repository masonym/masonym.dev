...
export const ENHANCE_MODE_STARS = Object.keys(ENHANCE_MODE).map(Number).sort((a, b) => a - b);
...
export function getEnhanceModeInfo(star, mode) {
    ...
    if (modes && mode >= 2) {
        ...
        return { mult: m.mult, success: m.success, boom: m.boom, maintain: 1 - m.success - m.boom };
    }
    const base = STAR_FORCE_RATES[star];
    return { mult: 1, success: base.success, boom: base.destroy, maintain: base.maintain };
}
...
export function evaluateStrategy(table, startingStar, targetStar) {
    ...
    for (let iter = 0; iter < 10000; iter++) {
        ...
        for (let s = targetStar - 1; s >= 0; s--) {
            ...
            const vc = (e.cost + e.pSuccess * Vc[next] + e.pBoom * Vc[e.recoverStar]) / denom;
            const vb = (e.pSuccess * Vb[next] + e.pBoom * (1 + Vb[e.recoverStar])) / denom;
    ...
    for (let iter = 0; iter < 10000; iter++) {
        ...
        for (let s = targetStar - 1; s >= 0; s--) {
            ...
            for (let i = 0; i < list.length; i++) {
                const e = list[i];
                const next = Math.min(s + e.successStars, targetStar);
                const v = (e.cost + e.pSuccess * V[next] + e.pBoom * (spareCost + V[e.recoverStar])) / (1 - e.pMaintain);
                if (v < best) {
                    best = v;
                    bestMode = i + 1;
                }
...
export function computeEnhanceFrontier({
    ...
    const ctx = { level, startingStar, targetStar, starCatchStars, eventTypes, mvpType };
    ...
    const optimizeAt = (lambda) => {
        ...
        const table = buildStarTable({ level, enhanceModes: modes, starCatchStars, eventTypes, mvpType });
        const { expectedCost, expectedBooms } = evaluateStrategy(table, startingStar, targetStar);
        return { modes, key: keyOf(modes), expectedCost, expectedBooms };
    };

    const lo = optimizeAt(0);
    const hi = optimizeAt(1e15);
    const points = new Map([[lo.key, lo], [hi.key, hi]]);
    ...
    const sorted = [...points.values()].sort(
        (x, y) => x.expectedCost - y.expectedCost || x.expectedBooms - y.expectedBooms
    );
    const frontier = [];
    ...
    for (let i = 0; i < frontier.length; i++) {
        ...
        frontier[i].spareFrom = prev
            ? (frontier[i].expectedCost - prev.expectedCost) / (prev.expectedBooms - frontier[i].expectedBooms)
            : 0;
        frontier[i].spareTo = next
            ? (next.expectedCost - frontier[i].expectedCost) / (frontier[i].expectedBooms - next.expectedBooms)
            : Infinity;
    }
    return frontier;
}
