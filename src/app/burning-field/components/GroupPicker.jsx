...
import { Compass, Loader2, MapPin, Plus, Search, Ticket, Users } from 'lucide-react';
...
export default function GroupPicker({ myGroups, loading, onSelect, onChanged, defaultIgn }) {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      const { data, error: rpcError } = await supabase.rpc('burning_public_groups', {
  ...
  return (
    <div className="space-y-4">
      ...
      {tab === 'mine' && (
        loading ? (
          ...
          <p className="text-primary-dim text-sm">
            You are not in any groups yet. Create one for the map you train on, or join
            someone else&apos;s with their invite code.
          </p>
        ) : (
              ...
              <li key={group.id}>
                ...
                >
                  <div className="flex items-center justify-between gap-2">
                    <span className="text-primary-bright font-bold truncate">{group.name}</span>
                    <span className="text-xs text-primary-dim">{group.role}</span>
                  </div>
      ...
      {tab === 'browse' && (
        <div className="space-y-3">
          ...
          {browsing ? (
            ...
            <p className="text-primary-dim text-sm">No public groups match that.</p>
          ) : (
                ...
                >
                  <div className="min-w-0">
                    ...
                    <p className="text-sm text-primary-dim truncate">
                      {group.map_name} · {group.world} · {group.channel_count} ch ·{' '}
      ...
      {tab === 'join' && (
        <form onSubmit={handleJoinByCode} className="flex flex-wrap items-center gap-2 max-w-md">
          <input
            value={code}
