...
const API_URL =
  "https://yaiphhwge8.execute-api.us-west-2.amazonaws.com/prod/query-items-by-date";
...
function AdvancedItemList() {
  ...
  const categorizeAndSortItems = useMemo(
    () => (items) => {
  ...
  useEffect(() => {
    const filterItems = () => {
      const now = new Date();
      const filtered = Object.entries(categorizedItems).reduce(
        (acc, [dateKey, items]) => {
          const filteredItems = items.filter(({ item }) => {
            ...
            const worldCondition =
              !filters.worldFilter ||
              (filters.worldFilter === "intWorlds"
                ? item.gameWorld
                    .split("/")
                    .some((id) => INT_WORLDS.includes(Number(id)))
                : filters.worldFilter === "heroWorlds"
                  ? item.gameWorld
                      .split("/")
                      .some((id) => HERO_WORLDS.includes(Number(id)))
                  : true);
  ...
  return (
    <div
      className="flex flex-col min-h-dvh h-auto pb-20 bg-cs-bg"
      style={{ backgroundAttachment: "fixed" }}
    >
      <h1 className="text-center text-3xl mb-0 mt-16 text-primary-bright font-bold">
        Cash Shop Sales
      </h1>
      ...
      {loading ? (
        <div className="flex justify-center items-center h-screen w-full text-center text-2xl text-gray-700 bg-white bg-opacity-80 absolute top-0 left-0 z-50">
          <p className="m-0 p-5 bg-white rounded-lg shadow-md">
            Loading items...
          </p>
        </div>
      ) : noItems ? (
            ...
            <div className="flex justify-center my-8">
              <button
                onClick={() => {
                  ...
                  const sortedDates =
                    viewMode === "past" ? allDates.reverse() : allDates;
                  setDisplayedDates(
                    sortedDates.slice(0, displayedDates.length + itemsPerPage),
                  );
                }}
              ...
              >
                Load More (
                {Object.keys(filteredCategories).length - displayedDates.length}{" "}
                remaining)
              </button>
            </div>
          )}
