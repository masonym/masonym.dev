  ...
  return (
    <div className="flex flex-col gap-4 mb-6">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <Link
          href={`/changelog${currentCategory ? `?category=${encodeURIComponent(currentCategory)}` : ""}`}
          className={`px-3 py-1 rounded-full border ${!currentTool ? "bg-primary text-background" : "hover:bg-primary-dark"}`}
        >
          All
        </Link>
        {tools.map((t) => {
          ...
          return (
            <Link
              key={t}
              href={`/changelog?${qs.toString()}`}
              className={`px-3 py-1 rounded-full border ${currentTool === t ? "bg-primary text-background" : "hover:bg-primary-dark"}`}
            >
              {t}
            </Link>
          );
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <Link
          href={`/changelog${currentTool ? `?tool=${encodeURIComponent(currentTool)}` : ""}`}
          className={`px-3 py-1 rounded-full border ${!currentCategory ? "bg-primary text-background" : "hover:bg-primary-dark"}`}
        >
          All
        </Link>
        {categories.map((c) => {
          ...
          return (
            <Link
              key={c}
              href={`/changelog?${qs.toString()}`}
              className={`px-3 py-1 rounded-full border capitalize ${currentCategory === c ? "bg-primary text-background" : "hover:bg-primary-dark"}`}
            >
              {c}
            </Link>
          );
...
export default function ChangelogClient() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        console.error("Failed to fetch /changelog.json", e);
      }
    }
    load();
    return () => {
  ...
  }, [entries, tool, category]);

  const tools = useMemo(() => {
    return [...new Set(entries.map((e) => e.tool).filter(Boolean))].sort();
  }, [entries]);
        ...
        {filtered.map((e) => (
          <li key={e.slug} className="border rounded-lg p-4 bg-background-bright/30">
            <div className="flex flex-wrap items-center gap-2 mb-2">
              {e.category && (
                <span className="text-xs uppercase tracking-wide px-2 py-0.5 rounded bg-primary-dark">
                  {e.category}
                </span>
              )}
              {e.tool && (
                <Link
                  href={`/changelog?tool=${encodeURIComponent(e.tool)}`}
                  className="text-xs px-2 py-0.5 rounded border hover:bg-primary-dark"
                >
                  {e.tool}
                </Link>
              )}
              {e.date && (
                <span className="text-xs text-foreground/70 ml-auto">
                  {String(e.date).includes("T") ? String(e.date).split("T")[0] : String(e.date)}
                </span>
              )}
            </div>
