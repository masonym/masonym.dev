...
const getStoneLabelPath = (stone) => {
  const label =
    stone.costType === "rushEnd"
      ? "rushEnd"
      : stone.category === "SHINE"
        ? "shine"
        : stone.category;
...
const isNodeUnlockable = (node, nodeLevels) => {
  const andMet = node.prereqAnd.every(
    (nodeIndex) => (nodeLevels[nodeIndex] || 0) > 0,
  );
  const orMet =
    node.prereqOr.length === 0 ||
    node.prereqOr.some((nodeIndex) => (nodeLevels[nodeIndex] || 0) > 0);
  return andMet && orMet;
};
...
const getTransitionCost = (stone, fromLevel) => {
  ...
  if (fromLevel === 0) {
    ...
    return formatCost(
      erdaLinkData.costs.activation?.[stone.category]?.[costType],
    );
  }
  return formatCost(
    erdaLinkData.costs.enforcement?.[stone.category]?.[fromLevel]?.[costType],
  );
};
...
const calcShineStepCosts = (shineStone) => {
  ...
  for (let level = 0; level < shineStone.maxLevel; level += 1) {
    ...
    const prev =
      level > 0 ? steps[level - 1] : { solErda: 0, fragments: 0, meso: 0 };
    steps[level] =
      success > 0
        ? {
            solErda: (attempt.solErda + downgrade * prev.solErda) / success,
            fragments:
              (attempt.fragments + downgrade * prev.fragments) / success,
  ...
  shineStone.passives
    .map((group) => {
      const key = Object.keys(
        group?.[1] || group?.[shineStone.maxLevel] || {},
      )[0];
...
const CostPill = ({ label, value, formatValue }) => (
  <div className="rounded-lg border border-primary-dim bg-primary-dark px-3 py-2 text-center min-w-[110px]">
    <div className="flex h-8 items-center justify-center">
      {label === "Sol Erda" && (
        <img src={solErda.src} alt="Sol Erda" className="h-8 w-8" />
      )}
      {label === "Fragments" && (
        <img
          src={solErdaFragment.src}
          alt="Sol Erda Fragment"
          className="h-8 w-8"
        />
      )}
      {label !== "Sol Erda" && label !== "Fragments" && (
        <span className="text-xs uppercase tracking-wide text-primary-dim">
          {label}
        </span>
      )}
    </div>
  ...
  <div className="flex flex-wrap items-center gap-x-4 gap-y-1">
    ...
    {usesMeso && (
      <span className="text-sm font-semibold text-primary-bright">
        {Math.round(cost.meso).toLocaleString()} meso
      </span>
    )}
  </div>
);
  ...
  return (
    <div className="group relative">
      ...
      <div className="invisible absolute right-0 top-full z-30 mt-1 w-64 rounded-lg border border-primary-dim bg-primary-dark p-3 opacity-0 shadow-xl transition group-hover:visible group-hover:opacity-100">
        <div className="mb-2 text-xs font-semibold uppercase tracking-wide text-primary-dim">
          Cost per level
        </div>
        <table className="w-full text-xs text-primary">
          ...
          <tbody>
            {rows.map((row) => (
              <tr key={row.to} className="border-t border-primary-dim/40">
                ...
                <td className="py-1 text-right">
                  {row.cost.solErda.toLocaleString()}
                </td>
                <td className="py-1 text-right">
                  {row.cost.fragments.toLocaleString()}
                </td>
                {usesMeso && (
                  <td className="py-1 text-right">
                    {row.cost.meso.toLocaleString()}
                  </td>
                )}
              </tr>
            ))}
            <tr className="border-t border-primary-dim font-semibold text-primary-bright">
              ...
              <td className="py-1 text-right">
                {total.solErda.toLocaleString()}
              </td>
              <td className="py-1 text-right">
                {total.fragments.toLocaleString()}
              </td>
              {usesMeso && (
                <td className="py-1 text-right">
                  {total.meso.toLocaleString()}
                </td>
              )}
            </tr>
          </tbody>
          ...
          {Array.from({ length: shineStone.maxLevel }, (_, level) => {
            ...
            return (
              <tr key={level} className="border-t border-primary-dim/40">
                <td className="py-1 text-left text-primary-bright">
                  {level} → {level + 1}
                </td>
          ...
          <tbody>
            {steps.map((step, level) => (
              <tr key={level} className="border-t border-primary-dim/40">
                ...
                <td className="py-1 text-right">
                  {Math.round(step.fragments).toLocaleString()}
                </td>
            ...
            <tr className="border-t border-primary-dim font-semibold text-primary-bright">
              <td className="py-1 text-left">Total</td>
              <td className="py-1 text-right">
                {Math.round(total.fragments).toLocaleString()}
              </td>
...
const ShineCalculator = ({ selectedClass }) => {
  ...
  const stoneMap = useMemo(
    () =>
      new Map(
        erdaLinkData.stones
          .filter((s) => s.treeId === character?.treeId)
          .map((s) => [s.id, s]),
      ),
    [character],
  );
  ...
  const shineStoneMap = useMemo(
    () =>
      new Map(
        erdaLinkData.shineStones
          .filter((stone) => stone.treeId === character?.treeId)
  ...
  const regularNodes = useMemo(
    () =>
      character?.nodes.filter(
        (node) => node.sector !== "SHINE" && node.position,
      ) || [],
    [character],
  );
  const shineNodes = useMemo(
    () => character?.nodes.filter((node) => node.sector === "SHINE") || [],
    [character],
  );
  const selectedNode = useMemo(
    () =>
      character?.nodes.find((node) => node.nodeIndex === selectedNodeIndex) ||
      regularNodes[0] ||
      null,
    [character, regularNodes, selectedNodeIndex],
  );
  const nodeMap = useMemo(
    () => new Map(character?.nodes.map((node) => [node.nodeIndex, node]) || []),
    [character],
  );
  const defaultNodeLevels = useMemo(() => {
    const defaults = {};
    character?.nodes.forEach((node) => {
      if (isFreeOrigin(stoneMap.get(node.stoneId)))
        defaults[node.nodeIndex] = 1;
    });
    return defaults;
  }, [character, stoneMap]);
  const normalizeLevels = useCallback(
    (levels = {}, includeDefaults = false) => {
      const normalized = includeDefaults
        ? { ...defaultNodeLevels, ...levels }
        : { ...levels };
      character?.nodes.forEach((node) => {
        const stone = stoneMap.get(node.stoneId);
        if (!stone) return;
        const level = clampLevel(
          normalized[node.nodeIndex] || 0,
          stone.maxLevel,
          minLevelFor(stone),
        );
  ...
  useEffect(() => {
    ...
    localStorage.setItem(
      `${STORAGE_PREFIX}_${selectedClass}`,
      JSON.stringify({ nodeLevels, goalLevels }),
    );
  ...
  const applyPrerequisiteLevels = (node, levels, externalLevels = {}) => {
    ...
    const isActive = (workingLevels, nodeIndex) =>
      (workingLevels[nodeIndex] || 0) > 0 ||
      (externalLevels[nodeIndex] || 0) > 0;
    ...
    const activateNode = (currentNode, currentLevels, path = new Set()) => {
      ...
      if (
        currentNode.prereqOr.length > 0 &&
        !currentNode.prereqOr.some((prereqIndex) =>
          isActive(nextLevels, prereqIndex),
        )
      ) {
        const cheapestRoute = currentNode.prereqOr.reduce(
          (cheapest, prereqIndex) => {
            const prereqNode = nodeMap.get(prereqIndex);
            ...
            return !cheapest ||
              compareCosts(result.addedCost, cheapest.addedCost) < 0
              ? result
              : cheapest;
          },
          null,
      ...
      addedCost = addCost(
        addedCost,
        stone ? calculateCostToLevel(stone, 1) : {},
      );
    ...
    node.prereqAnd.forEach((prereqIndex) => {
      const result = activateNode(nodeMap.get(prereqIndex), {
        ...updatedLevels,
      });
      if (!result) return;
    ...
    if (
      node.prereqOr.length > 0 &&
      !node.prereqOr.some((prereqIndex) => isActive(updatedLevels, prereqIndex))
    ) {
  ...
  const setNodeLevel = (
    ...
    value,
    setter,
    shouldApplyPrerequisites = false,
    externalLevels = {},
  ) => {
    const stone = stoneMap.get(node.stoneId);
    if (!stone) return;
    ...
    setter((previous) => {
      const updatedLevels =
        shouldApplyPrerequisites && level > 0
          ? applyPrerequisiteLevels(node, previous, externalLevels)
          : { ...previous };
      if (level > 0 || isFreeOrigin(stone)) {
  ...
  const adjustCurrentLevel = (node, amount) => {
    ...
    const nextLevel = clampLevel(
      (nodeLevels[node.nodeIndex] || 0) + amount,
      stone.maxLevel,
      minLevelFor(stone),
    );
  ...
  const resolvedGoalLevels = allNodes.reduce((resolved, node) => {
    ...
    return applyPrerequisiteLevels(
      node,
      { ...resolved, [node.nodeIndex]: goalLevel },
      nodeLevels,
    );
  }, {});

  const spentTotal = allNodes.reduce(
    (total, node) => {
      const currentLevel = nodeLevels[node.nodeIndex] || 0;
      if (node.sector === "SHINE") {
        ...
        return addCost(
          total,
          sumShineCost(calcShineStepCosts(shineStone), 0, currentLevel),
        );
      }
      const stone = stoneMap.get(node.stoneId);
  ...
  const remainingTotal = allNodes.reduce(
    (total, node) => {
      const currentLevel = nodeLevels[node.nodeIndex] || 0;
      const goalLevel = Math.max(
        currentLevel,
        resolvedGoalLevels[node.nodeIndex] || goalLevels[node.nodeIndex] || 0,
      );
      if (node.sector === "SHINE") {
        const shineStone = shineStoneMap.get(node.stoneId);
        if (!shineStone) return total;
        return addCost(
          total,
          sumShineCost(calcShineStepCosts(shineStone), currentLevel, goalLevel),
        );
      }
      const stone = stoneMap.get(node.stoneId);
      if (!stone) return total;
  ...
  const renderNode = (node) => {
    ...
    return (
      ...
      >
        <img
          src={getStoneIconPath(stone, character.treeId, !activated)}
          alt={stone.name}
  ...
  const selectedStone = selectedNode
    ? stoneMap.get(selectedNode.stoneId)
    : null;
  const selectedCurrentLevel = selectedNode
    ? nodeLevels[selectedNode.nodeIndex] || 0
    : 0;
  const selectedGoalLevel = selectedNode
    ? (goalLevels[selectedNode.nodeIndex] ?? selectedCurrentLevel)
    : 0;
  const selectedResolvedGoalLevel = selectedNode
    ? Math.max(
        selectedCurrentLevel,
        resolvedGoalLevels[selectedNode.nodeIndex] || selectedGoalLevel,
      )
    : 0;
  const selectedCurrentCost = selectedStone
    ? calculateCostToLevel(selectedStone, selectedCurrentLevel)
    : { solErda: 0, fragments: 0, meso: 0 };
  const selectedGoalCost = selectedStone
    ? calculateDeltaCost(
        selectedStone,
        selectedCurrentLevel,
        selectedResolvedGoalLevel,
      )
    : { solErda: 0, fragments: 0, meso: 0 };
  ...
  const selectedAtMaxLevel = selectedStone
    ? selectedCurrentLevel >= selectedStone.maxLevel
    : false;
  const selectedNextLevelCost =
    selectedStone && !selectedAtMaxLevel
      ? getTransitionCost(selectedStone, selectedCurrentLevel)
      : null;

  // SHINE stone detail (RNG / expected-cost variant of the panel above).
  const selectedShineStone =
    selectedNode?.sector === "SHINE"
      ? shineStoneMap.get(selectedNode.stoneId)
      : null;
  const shineSteps = selectedShineStone
    ? calcShineStepCosts(selectedShineStone)
    : null;
  const shineSpentCost = selectedShineStone
    ? sumShineCost(shineSteps, 0, selectedCurrentLevel)
    : null;
  const shineGoalCost = selectedShineStone
    ? sumShineCost(shineSteps, selectedCurrentLevel, selectedResolvedGoalLevel)
    : null;
  const shineCurrentStats = selectedShineStone
    ? getStatsAtLevel(selectedShineStone, selectedCurrentLevel)
    : {};
  const shineAtMaxLevel = selectedShineStone
    ? selectedCurrentLevel >= selectedShineStone.maxLevel
    : false;
  const shineNextLevelCost =
    selectedShineStone && !shineAtMaxLevel
      ? shineSteps[selectedCurrentLevel]
      : null;
    ...
    <div className="flex w-full max-w-[1800px] flex-col gap-6 px-2 py-4">
      <div className="flex flex-col items-center gap-3 text-center">
        <div className="flex items-center gap-3">
          <h2 className="text-[32px] text-primary-bright">
            {selectedClass} Erda Link Calculator
          </h2>
          <button
            type="button"
            onClick={resetBuild}
        ...
        <p className="max-w-3xl text-sm text-primary">
          Click a stone on the board to enter your current level and goal level.
          Yellow stones are available from current prerequisites, green stones
          are activated, and dim stones are locked.
        </p>
        <p className="max-w-3xl text-sm text-primary">
          Light Module functionaity is not implemented. I may add it later, but
          for now, nodes with no requirement aside from being within the Light
          Module will be unlocked. Sorry!
        </p>
      </div>

      <div className="grid gap-3 md:grid-cols-2">
        <div className="rounded-xl border border-primary-dim bg-background p-4">
          <h3 className="mb-3 text-center text-xl font-semibold text-primary-bright">
            Materials Spent
          </h3>
          <div className="flex flex-wrap justify-center gap-3">
            ...
            {spentTotal.meso > 0 && (
              <CostPill
                label="Meso"
                value={spentTotal.meso}
                formatValue={formatMeso}
              />
            )}
          </div>
        </div>
        <div className="rounded-xl border border-primary-dim bg-background p-4">
          ...
          <div className="flex flex-wrap justify-center gap-3">
            ...
            {remainingTotal.meso > 0 && (
              <CostPill
                label="Meso"
                value={remainingTotal.meso}
                formatValue={formatMeso}
              />
            )}
          </div>
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_360px]">
        <div className="rounded-xl border border-primary-dim bg-primary-dark/40 p-3">
          ...
          >
            <div
              className="relative min-w-[1800px]"
              style={{ width: CANVAS_WIDTH, height: CANVAS_HEIGHT }}
            >
              <img
                src="/erda-link/erdalink-background.png"
                alt="Erda Link board"
        ...
        <div className="flex flex-col gap-4">
          {selectedStone && selectedNode && (
            <div className="rounded-xl border border-primary-dim bg-background p-4">
              <div className="mb-4 flex items-center gap-3">
                <img
                  src={getStoneIconPath(
                    selectedStone,
                    character.treeId,
                    selectedCurrentLevel === 0,
                  )}
                ...
                <div>
                  ...
                  <img
                    src={getStoneLabelPath(selectedStone)}
                    alt={selectedStone.category}
                    className="mt-1 h-5 w-auto"
                  />
                </div>
              </div>
              <p className="mb-4 text-sm text-primary">
                {parseDesc(selectedStone.desc)}
              </p>
              ...
              <div className="mt-4 rounded-lg border border-primary-dim bg-primary-dark/40 px-3 py-2">
                <div className="flex items-center justify-between gap-2">
                  ...
                  <CostTableTooltip
                    stone={selectedStone}
                    usesMeso={selectedStoneUsesMeso}
                  />
                </div>
                {selectedNextLevelCost && (
                  <div className="mt-2">
                    <CostInline
                      cost={selectedNextLevelCost}
                      usesMeso={selectedStoneUsesMeso}
                    />
                  </div>
                )}
              ...
              <div className="mt-4 grid gap-3">
                <div>
                  <h4 className="mb-2 text-sm font-semibold text-primary-bright">
                    Spent on this stone
                  </h4>
                  <div className="flex flex-wrap gap-2">
                    <CostPill
                      label="Sol Erda"
                      value={selectedCurrentCost.solErda}
                    />
                    <CostPill
                      label="Fragments"
                      value={selectedCurrentCost.fragments}
                    />
                    {selectedStoneUsesMeso && (
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    <CostPill
                      label="Sol Erda"
                      value={selectedGoalCost.solErda}
                    />
                    <CostPill
                      label="Fragments"
                      value={selectedGoalCost.fragments}
                    />
                    {selectedStoneUsesMeso && (
          ...
          {selectedShineStone && selectedNode && (
            <div className="rounded-xl border border-primary-dim bg-background p-4">
              <div className="mb-3 flex items-center gap-3">
                <img
                  src={getStoneLabelPath({ category: "SHINE" })}
                  alt="Shine"
                  className="h-5 w-auto"
                />
                <h3 className="text-lg font-semibold text-primary-bright">
                  {getShineStoneTitle(selectedShineStone)}
                </h3>
              </div>
              {selectedShineStone.conditions.length > 0 && (
                <p className="mb-2 text-xs text-primary-dim">
                  Unlock: {formatShineConditions(selectedShineStone.conditions)}
                </p>
              )}
              <p className="mb-4 text-xs text-primary-dim">
                Enforcement is RNG - costs shown are the{" "}
                <span className="text-primary">expected (average)</span>{" "}
                materials.
              </p>
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className="font-semibold text-primary-bright">
                        {formatStatValue(key, shineCurrentStats[key] || 0)}
                      </span>
                    </div>
                  );
                })}
                ...
                <label className="text-sm text-primary">
                  ...
                  <div className="mt-1 flex items-center gap-2">
                    ...
                    <input
                      ...
                      onBlur={() => {
                        if (goalDraft !== null)
                          setShineLevel(selectedNode, goalDraft, setGoalLevels);
                        setGoalDraft(null);
                      }}
                      className="w-full rounded border border-primary-dim bg-primary-dark p-2 text-center text-primary-bright"
                    />
              ...
              <div className="mt-4 rounded-lg border border-primary-dim bg-primary-dark/40 px-3 py-2">
                <div className="flex items-center justify-between gap-2">
                  ...
                  <div className="flex gap-2">
                    ...
                    <ShineCostTableTooltip
                      shineStone={selectedShineStone}
                      steps={shineSteps}
                    />
                  </div>
                </div>
                {shineNextLevelCost && (
                  <div className="mt-2 flex items-center gap-3">
                    ...
                    <span className="text-xs text-primary-dim">
                      @{" "}
                      {
                        getShineProb(selectedShineStone, selectedCurrentLevel)
                          .successRate
                      }
                      % success
                    </span>
              ...
              <div className="mt-4 grid gap-3">
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    <CostPill
                      label="Fragments"
                      value={shineSpentCost.fragments}
                    />
                    <CostPill
                      label="Meso"
                      value={shineSpentCost.meso}
                      formatValue={formatMeso}
                    />
                  </div>
                </div>
                <div>
                  <h4 className="mb-2 text-sm font-semibold text-primary-bright">
                    Expected remaining for goal
                  </h4>
                  <div className="flex flex-wrap gap-2">
                    <CostPill label="Sol Erda" value={shineGoalCost.solErda} />
                    <CostPill
                      label="Fragments"
                      value={shineGoalCost.fragments}
                    />
                    <CostPill
                      label="Meso"
                      value={shineGoalCost.meso}
                      formatValue={formatMeso}
                    />
                  </div>
                </div>
          ...
          <div className="rounded-xl border border-primary-dim bg-background p-4">
            <h3 className="mb-3 text-lg font-semibold text-primary-bright">
              SHINE Stones
            </h3>
              ...
              {shineNodes.map((node) => {
                ...
                return (
                  ...
                  >
                    <div className="flex items-center justify-between gap-2">
                      <span className="font-semibold text-primary-bright">
                        {getShineStoneTitle(shineStone)}
                      </span>
                      <span className="text-xs text-primary">
                        {level}/{shineStone.maxLevel}
                      </span>
                    </div>
                    {level < shineStone.maxLevel && nextProb && (
                      <div className="mt-1 text-xs text-primary-dim">
                        Next: {nextProb.successRate}% success
                      </div>
                    )}
                  </button>
                );
