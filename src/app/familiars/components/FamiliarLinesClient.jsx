...
const categorySortKey = (line) => {
  ...
  return (systemIndex < 0 ? 99 : systemIndex) * 100 + (rarityIndex < 0 ? 99 : rarityIndex);
};
...
const FamiliarLinesClient = () => {
  ...
  const filteredAndSorted = useMemo(() => {
    ...
    return result;
  }, [lines, searchTerm, selectedSystems, selectedRarities]);
  ...
  const groupedRows = useMemo(() => {
    ...
    sortedGroups.forEach(([label, group]) => {
      rows.push({ type: 'header', label, count: group.lines.length, sortKey: group.sortKey });
  ...
  }, [searchTerm, selectedSystems, selectedRarities]);
    ...
    observerRef.current = new IntersectionObserver(
      (entries) => {
        if (entries[0].isIntersecting) {
          setDisplayCount((prev) => Math.min(prev + ITEMS_PER_PAGE, groupedRows.length));
        }
      },
      { threshold: 0.1 }
    );
  ...
  const renderLine = (line) => {
    ...
    const levelOptions = levelKeys.map((k) => ({ value: Number(k), label: `Level ${k}` }));
    const currentLevel = selectedLevels[line.id] ?? (levelOptions[0] ? levelOptions[0].value : 1);
      ...
      >
        <div className="flex flex-wrap items-baseline gap-x-3 gap-y-1.5">
          <p className="text-[var(--primary-bright)] text-base leading-snug min-w-0 flex-1">
            {formatDescription(line.description, stats)}
          </p>
          ...
          <div className="flex flex-wrap items-center gap-2 shrink-0">
            {endTimeFormatted && (
              ...
              >
                ...
                <span>EVENT</span>
                <span className="normal-case text-amber-200">· Ends {endTimeFormatted}</span>
              </div>
            )}
            <span
              className={`
        ...
        {levelOptions.length > 1 && (
          <div className="mt-2 flex items-center gap-2">
            <FilterDropdown
              ...
              onChange={(val) => setSelectedLevels((prev) => ({ ...prev, [line.id]: val }))}
              valueKey="value"
              labelKey="label"
            />
            {!hasStats && (
              <span className="text-xs text-[var(--primary-dim)]">No level data</span>
            )}
          </div>
        )}
      </div>
    ...
    <div className="container mx-auto px-4 py-8 max-w-7xl">
      <div className="text-center mb-8">
        ...
        <p className="text-lg text-[var(--primary)] mt-2 max-w-2xl mx-auto">
          Browse familiar potentials and Mystic Frontier lines by rarity and system.
        </p>
      </div>
      ...
      <div className="mb-4 flex items-center justify-between text-sm text-[var(--primary)]">
        <span>
          Showing {showingCount.toLocaleString()} of {filteredCount.toLocaleString()} lines
        </span>
      ...
      {displayedRows.length > 0 ? (
              ...
              row.type === 'header' ? (
                ...
                >
                  <div className="flex items-center justify-between">
                    <h2 className="text-lg font-semibold text-[var(--secondary)]">{row.label}</h2>
                    <span className="text-xs text-[var(--primary-dim)]">{row.count} lines</span>
                  </div>
                </div>
              ) : (
                renderLine(row.line)
        ...
        <div className="text-center py-16">
          ...
          <p className="text-[var(--primary)] text-lg">No lines match your filters</p>
          <button
      ...
      <footer className="text-center mt-12 text-[var(--primary-dim)] text-sm">
        <p>
          Familiar line data is extracted from game files. Placeholder values are filled from per-level stats when available.
        </p>
      </footer>
    </div>
  );
