...
const eq = (label, got, want) => {
  ...
  if (!ok) fails += 1;
  console.log(
    `${ok ? "  ok  " : " FAIL "} ${label}: got ${JSON.stringify(got)}${ok ? "" : `, want ${JSON.stringify(want)}`}`,
  );
};
...
const pct = (level, tier, advantaged) =>
  weaponAttackFlame(10000, level, tier, advantaged) / 100;

eq(
  "lv120-159 ordinary t1..t5",
  [1, 2, 3, 4, 5].map((t) => pct(120, t, false)),
  [4, 8.8, 14.52, 21.3, 29.29],
);
eq(
  "lv200-249 ordinary t1..t5",
  [1, 2, 3, 4, 5].map((t) => pct(200, t, false)),
  [6, 13.2, 21.78, 31.95, 43.93],
);
eq(
  "lv200-249 boss flame t3..t7",
  [3, 4, 5, 6, 7].map((t) => pct(200, t, true)),
  [18, 26.4, 36.3, 47.92, 61.5],
);
eq("lv250+ boss flame t7", pct(250, 7, true), 71.75);
eq(
  "no attack flame without base attack",
  weaponAttackFlame(0, 200, 7, true),
  0,
);
...
eq(
  "ordinary and boss flame differ at t5",
  pct(200, 5, false) === pct(200, 5, true),
  false,
);
...
eq(
  "flat +1 per tier",
  [1, 2, 3, 4, 5, 6, 7].map(nonWeaponAttackFlame),
  [1, 2, 3, 4, 5, 6, 7],
);
...
eq(
  "boss damage rolls on weapons",
  flameLinesFor(weapon).includes("boss"),
  true,
);
...
eq(
  "DEF rolls like any other single stat",
  flameLinesFor(hat).includes("def"),
  true,
);
eq(
  "All Stat needs level 70 off a weapon",
  flameLinesFor(lowHat).includes("allStat"),
  false,
);
eq(
  "All Stat rolls on a level 200 hat",
  flameLinesFor(hat).includes("allStat"),
  true,
);

// A hat with no attack of its own still rolls the flat attack line, all 7 tiers.
eq(
  "a hat with no attack still rolls attack",
  [1, 4, 7].map(
    (t) => flameLineValue("att", t, { level: 200, isWeapon: false })?.value,
  ),
  [1, 4, 7],
);
// An ordinary weapon's attack line stops at tier 5; an advantaged one starts at 3.
eq(
  "ordinary weapon attack stops at t5",
  flameLineValue("att", 6, { level: 200, baseAttack: 276, isWeapon: true }),
  null,
);
eq(
  "advantaged weapon attack starts at t3",
  flameLineValue("att", 2, {
    level: 200,
    baseAttack: 276,
    isWeapon: true,
    advantaged: true,
...
eq(
  "advantaged weapon attack reaches t7",
  flameLineValue("att", 7, {
    level: 200,
    baseAttack: 276,
    isWeapon: true,
    advantaged: true,
...
eq(
  "  Max HP @15*",
  starForceGains({ level: 200, stars: 15, slot: "Cp" }).hp,
  5 * 2 + 10 * 3 + 15 * 2 + 20 * 2 + 25 * 6,
);
eq(
  "  gloves take no Max HP",
  starForceGains({ level: 200, stars: 15, slot: "Gv" }).hp,
  0,
);
eq(
  "  eye accessories take no Max HP",
  starForceGains({ level: 200, stars: 15, slot: "Ay" }).hp,
  0,
);
...
eq(
  "  sub-15 compounding att",
  starForceWeaponAttack(276, 15),
  wep22.att - (13 + 13 + 14 + 14 + 15 + 16 + 17),
);
// A weapon's third star is still worth 5 Max HP where armour's is already 10.
eq(
  "  weapon Max HP @15*",
  starForceGains({ level: 200, stars: 15, slot: "WpSi" }).hp,
  5 * 3 + 10 * 2 + 15 * 2 + 20 * 2 + 25 * 6,
);

const glove = starForceGains({
  level: 200,
  stars: 15,
  slot: "Gv",
  baseAttack: 10,
...
const gloveNoAtt = starForceGains({
  level: 200,
  stars: 15,
  slot: "Gv",
  baseAttack: 0,
...
eq(
  "every stat moves together",
  [badge22.dex, badge22.int, badge22.luk],
