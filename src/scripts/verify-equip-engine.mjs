...
import {
  filterItemsForSlot, slotCandidates, byPower, itemScore, DEFAULT_FILTERS,
...
eq('no phantom row from float dust',
  diffStats({ values: {}, ied: [5] }, { values: {}, ied: [5] }), []);
eq('a real IED gain still shows',
  diffStats({ values: {}, ied: [5] }, { values: {}, ied: [5, 10] }).map((r) => r.key), ['ied']);
eq('one decimal, not seventeen', formatStat('ied', combineIed([30, 10])), '37%');
...
eq('legendary boss line @ lvl200 index', potentialValueAt(bossLine, 20), { boss: 30 });
eq('legendary boss line @ lvl250 index', potentialValueAt(bossLine, 25), { boss: 35 });

// ── Multi-slot items ─────────────────────────────────────────────────────────
console.log('\n=== multi-slot items ===');
const twoHander = items.find((i) => i.slot === 'WpSi');
const overall = items.find((i) => i.slot === 'MaPn');
eq('WpSi occupies weapon+secondary', occupiedSlots(twoHander, 'weapon'), ['weapon', 'secondary']);
eq('MaPn occupies top+bottom', occupiedSlots(overall, 'top'), ['top', 'bottom']);
...
eq('WpSi is only equippable as a weapon', equippableSlots(twoHander), ['weapon']);
eq('MaPn is only equippable as a top', equippableSlots(overall), ['top']);
eq('no two-hander fits secondary',
  items.filter((i) => i.slot === 'WpSi' && itemFitsSlot(i, 'secondary')).length, 0);
eq('no overall fits bottom',
  items.filter((i) => i.slot === 'MaPn' && itemFitsSlot(i, 'bottom')).length, 0);
...
console.log('\n=== emblems and hearts ===');
{
  ...
  eq('every emblem keeps its WZ islot', emblems.every((i) => i.islot === 'Si'), true);
  eq('emblems fit only the emblem slot',
    emblems.every((i) => itemFitsSlot(i, 'emblem') && !itemFitsSlot(i, 'secondary')), true);

  eq('hearts are present', hearts.length > 0, true);
  eq('every heart keeps its WZ islot', hearts.every((i) => i.islot === 'Tm'), true);
  eq('hearts fit the heart slot', hearts.every((i) => itemFitsSlot(i, 'heart')), true);
  ...
  eq('Total Control stats', { str: total.stats.str, att: total.stats.att, ied: total.stats.ied },
    { str: 25, att: 15, ied: 30 });

  // Every slot in the window must have something the picker can offer.
  const empty = LOADOUT_SLOTS
    .filter((s) => !items.some((i) => itemFitsSlot(i, s.key)))
    .map((s) => s.key);
...
console.log('\n=== badge star force ===');
{
  ...
  const at15 = flattenStats(resolveLoadout({ badge: { itemId: badge.id, stars: 15 } }, data).stats);
  const at22 = flattenStats(resolveLoadout({ badge: { itemId: badge.id, stars: 22 } }, data).stats);

  console.log(`  ${badge.name}: 15* att ${at15.att || 0}, 22* att ${at22.att || 0}`);
  ...
  const ring15 = flattenStats(resolveLoadout({ ring1: { itemId: ring.id, stars: 15 } }, data).stats);
  const ring22 = flattenStats(resolveLoadout({ ring1: { itemId: ring.id, stars: 22 } }, data).stats);
  eq('rings still gain attack past 15', (ring22.att || 0) > (ring15.att || 0), true);
}
...
console.log('\n=== class filter ===');
{
  ...
  eq('magician item hidden from warrior', itemMatchesClass(mageOnly, 'warrior'), false);
  eq('magician item shown to magician', itemMatchesClass(mageOnly, 'magician'), true);
  ...
  eq('Xenon gear hidden from warrior', itemMatchesClass(xenon, 'warrior'), false);

  // Items with no requirement are equippable by everyone.
  const unrestricted = items.find((i) => !i.reqJob);
  eq('unrestricted item shows for every class',
    ['warrior', 'magician', 'bowman', 'thief', 'pirate']
      .every((c) => itemMatchesClass(unrestricted, c)), true);
...
console.log('\n=== flame eligibility ===');
{
  ...
  for (const slot of ['Cp', 'Ma', 'Pn', 'So', 'Gv', 'Sr', 'Wp', 'WpSi', 'Af', 'Pe', 'Be']) {
  ...
  eq('Scarlet Shoulder is the shoulder exception', acceptsFlames(scarlet), true);
  ...
  const diff = diffLoadouts({ ring1: { itemId: ring.id } }, { ring1: flamed }, data);
...
console.log('\n=== preset potential ===');
{
  ...
  eq('Dominator Pendant has three preset lines', dominator.presetPotential.length, 3);
  const resolved = resolveLoadout({ pendant: { itemId: dominator.id } }, data);
  eq('preset potential is counted without being entered',
    flattenStats(resolved.stats).strP, 15);

  // Entering lines by hand replaces the preset rather than stacking on it.
  const overridden = resolveLoadout(
    { pendant: { itemId: dominator.id, potentials: [{ optionId: 40001 }] } }, data,
  );
  eq('an override replaces the preset', flattenStats(overridden.stats).strP, undefined);
}
...
console.log('\n=== item list filtering ===');
{
  ...
  eq('notability narrows without emptying',
    notable.length > 0 && notable.length < items.length, true);
  ...
  for (const { key } of LOADOUT_SLOTS) {
    for (const classKey of ['all', 'warrior', 'magician', 'bowman', 'thief', 'pirate']) {
      const shown = filterItemsForSlot(items, { slotKey: key, classKey, ...DEFAULT_FILTERS });
  ...
  const withJunk = filterItemsForSlot(items, {
    slotKey: 'hat', classKey: 'all', currentId: junk.id, notableOnly: true, minLevel: 200,
  });
  eq('the equipped item survives any filter',
    withJunk.some((i) => i.id === junk.id), true);
  ...
  const outranks = (slotKey, classKey, better, worse) => {
    const list = filterItemsForSlot(items, { slotKey, classKey, ...DEFAULT_FILTERS })
  ...
  outranks('secondary', 'bowman', "Princess No's Floral Jewel", 'Maple Treasure Ereve Brilliance');
  ...
  outranks('pendant', 'all', 'Superior Engraved Gollux Pendant', 'Reinforced Engraved Gollux Pendant');
  ...
  const blackHeart = items.find((i) => i.name === 'Black Heart' && i.stats.att === 77);
  eq('att and matt are not both counted', itemScore(blackHeart) < 77 * 5 + 100, true);
  eq('a magician scores it off MATT alone',
    itemScore(blackHeart, 'magician'), itemScore(blackHeart, 'warrior'));

  for (const key of ['hat', 'weapon', 'secondary', 'pendant', 'badge', 'ring1']) {
    const top = filterItemsForSlot(items, { slotKey: key, classKey: 'magician', ...DEFAULT_FILTERS })
      .sort(byPower('magician')).slice(0, 4).map((i) => i.name);
  ...
  eq('folded duplicates still resolve',
    data.itemIndex.get(aliased.aliases[0])?.id, aliased.id);
  const ids = new Set(items.map((i) => i.id));
  eq('no alias collides with a live id',
    items.flatMap((i) => i.aliases ?? []).filter((a) => ids.has(a)).length, 0);
}

// ── Set effects are cumulative ───────────────────────────────────────────────
console.log('\n=== set effects (Arcane Umbra Warrior, set 617) ===');
const umbra = sets.find((s) => s.id === 617);
const umbraItems = umbra.members.map((id) => data.itemIndex.get(id)).filter(Boolean);
...
for (const pieces of [2, 3, 4]) {
  ...
  console.log(`  ${pieces}-piece → att ${flat.att || 0}, boss ${flat.boss || 0}%, ied ${Math.round(flat.ied || 0)}%  (thresholds ${active[0]?.thresholds})`);
}
...
console.log('\n=== loadout diff: swapping out a set piece ===');
{
  ...
  const otherHat = items.find(
    (i) => i.slot === 'Cp' && i.reqLevel === 200 && i.setId !== 617 && i.stats.str > 0,
  );
  ...
  console.log(`  set pieces: ${result.before.sets[0]?.pieces} → ${result.after.sets.find((s) => s.setId === 617)?.pieces ?? 0}`);
  ...
  for (const r of result.rows.slice(0, 8)) {
    ...
    console.log(`    ${r.label.padEnd(18)} ${sign}${Math.round(r.delta * 10) / 10}`);
  }
...
console.log('\n=== full item resolution ===');
{
  ...
  const plain = flattenStats(resolveLoadout({ hat: { itemId: hat.id } }, data).stats);
  const decked = flattenStats(resolveLoadout({
    hat: {
      itemId: hat.id,
      stars: 22,
      flames: [{ line: 'str', tier: 7 }, { line: 'allStat', tier: 5 }],
      potentials: [{ optionId: 40041 }],
    },
  }, data).stats);

  console.log(`  base:   str ${plain.str}, att ${plain.att}, ied ${Math.round(plain.ied)}%`);
  console.log(`  22*+flame+pot: str ${decked.str}, att ${decked.att}, allStatP ${decked.allStatP || 0}%, strP ${decked.strP || 0}%`);
...
console.log('\n=== equip window layout ===');
{
  ...
  for (const slot of EQUIP_SLOT_LAYOUT) {
    ...
    if (slot.x < 0 || slot.y < 0
      ...
      || slot.y + SLOT_SIZE > WINDOW_HEIGHT) outOfBounds += 1;
  ...
  const unmapped = LOADOUT_SLOTS.filter((s) => !seenKeys.has(s.key)).map((s) => s.key);
...
console.log('\n=== star force ranges ===');
{
  ...
  eq('Genesis weapon is fixed at 22★', [starFloor(genesis), starCap(genesis)], [22, 22]);
  eq('a fixed-star item offers no choice', isStarForceable(genesis), false);

  const destiny = items.filter((i) => i.name === 'Destiny Sword')
    .sort((a, b) => starCap(a) - starCap(b));
  eq('two Destiny stages', destiny.length, 2);
  eq('Destiny stage 1 stops at 22★', [starFloor(destiny[0]), starCap(destiny[0])], [22, 22]);
  eq('Destiny stage 2 reaches 25★', [starFloor(destiny[1]), starCap(destiny[1])], [22, 25]);

  // The three Astra secondary grades share a name and differ only in stats.
  const astra = items.filter((i) => i.name === 'Astra Talisman').sort((a, b) => a.id - b.id);
  eq('three Astra grades', astra.map((i) => starCap(i)), [15, 20, 30]);
  eq('the weakest Astra grade is the lowest cap',
    astra[0].stats.int < astra[2].stats.int, true);
  ...
  eq('secondaries are exempt from the no-upgrade-slots rule',
    [princessNo.tuc, starCap(princessNo)], [0, 30]);

  const redBeryl = named('Red Beryl Pendant');
  eq('Red Beryl is granted at 20★', [starFloor(redBeryl), starCap(redBeryl)], [20, 20]);

  for (const [name, slot] of [['Genesis Badge', 'Ba'], ['Immortal Legacy', 'Me'],
    ['Mitra\'s Rage: Warrior', 'Em'], ['Inverse Codex', 'Po']]) {
  ...
  outranks('secondary', 'magician', 'Astra Talisman', "Princess No's Flaming Book");
}
...
console.log('\n=== preset gear (Red Beryl) ===');
{
  ...
  eq('no preset without a main stat to key it to', itemPreset(redBeryl, 'all'), null);

  const warrior = configForItem(redBeryl, 'warrior');
  eq('granted at 20★', warrior.stars, 20);
  eq('Unique 10% then two epic 7% main stat lines',
    warrior.potentials.map((p) => p.optionId), [30041, 20041, 20041]);
  eq('four tier-5 bonus stat lines',
    warrior.flames, [
      { line: 'str', tier: 5 }, { line: 'strDex', tier: 5 },
      { line: 'allStat', tier: 5 }, { line: 'att', tier: 5 },
    ]);

  const magician = configForItem(redBeryl, 'magician');
  eq('a magician gets INT and magic attack',
    magician.flames.map((f) => f.line), ['int', 'intLuk', 'allStat', 'matt']);
  eq('and INT potential lines',
    magician.potentials.map((p) => p.optionId), [30043, 20043, 20043]);

  // The preset must actually resolve, not just be recorded.
  const stats = flattenStats(resolveLoadout({ earrings: configForItem(redBeryl, 'warrior') }, data).stats);
  ...
  const kept = configForItem(ordinary, 'warrior', { stars: 17, flames: [{ line: 'str', tier: 7 }] });
  eq('a swap keeps the stars', kept.stars, 17);
  eq('a swap keeps the flames', kept.flames.length, 1);
  const clamped = configForItem(items.find((i) => i.name === 'Genesis Sword'), 'warrior', { stars: 30 });
...
console.log('\n=== exceptional enhancement ===');
{
  ...
  eq('every one of them says how many',
    withHammers.every((i) => exceptionalSlots(i) > 0), true);
  ...
  eq('the slot fallback reached Original Sin of Pride',
    pride.exceptional, berserked.exceptional);
  ...
  eq('one hammer', exceptionalGains(medal, 1),
    { matt: 15, att: 15, str: 20, dex: 20, int: 20, luk: 20, hp: 1000, mp: 1000 });
  ...
  const before = flattenStats(resolveLoadout({ medal: { itemId: medal.id } }, data).stats);
  const after = flattenStats(resolveLoadout({ medal: { itemId: medal.id, exceptional: 3 } }, data).stats);
...
console.log('\n=== badges ===');
{
  ...
  eq('exactly two badges can be star forced',
    starrable.map((i) => i.name).sort(), ['Ghost Ship Exorcist', 'Sengoku Hakase Badge']);
  eq('both cap at 22', starrable.map((i) => starCap(i)), [22, 22]);
  eq('and only those two take potential',
    badges.filter((i) => !i.noPotential).map((i) => i.name).sort(),
  ...
  const at22 = flattenStats(resolveLoadout({ badge: { itemId: hakase.id, stars: 22 } }, data).stats);
  const base = flattenStats(resolveLoadout({ badge: { itemId: hakase.id } }, data).stats);
...
console.log('\n=== flames on real items ===');
{
  ...
  eq('accessory attack flames are flat, not a share of base attack',
    [1, 4, 7].map((t) => flameLineValue('att', t, flameContext(earrings))?.value), [1, 4, 7]);
  eq('and the same on a hat with no attack at all',
    [7].map((t) => flameLineValue('att', t, flameContext(hat))?.value), [7]);

  const flamed = flattenStats(resolveItem(hat, {
    flames: [{ line: 'str', tier: 7 }, { line: 'att', tier: 7 }],
  }, data.lineIndex));
